...
Parallel: words are processed simultaneously. The cognitive system processes information from several words at once, allowing for faster and more efficient reading. Attention is distributed dynamically across several words, with a gradient of attention decreasing from the point of fixation outward. 
...
Skipping: length, predictability.
FFD: earliest point to observe an effect due to properties of a word, e.g., frequency.
GD: sensitive to semantic and syntactic anomalies.
:::
...
RPD: lexical and integration difficulties.
Regr-out: difficulties in higher-level processing (e.g., discourse processing) of a text.
:::
...
Re-reading time (RPD-GD): indication of the time spent resolving a problem.
TRT: impacted by both early and late processing. Effect in TRT but not in early measures -\> late effect on processing.
:::
...
Usually, exploration of how patterns over a IA differ as a function of its properties.
...
-   Line spacing[^4]
    -   Tracking less accurate for vertical movements
    -   Vertical drift
...
:::notes
Typical/atypical instance of a category. Plunger: to unclog the WC.
Matching is not important (large distance from the target).
:::
